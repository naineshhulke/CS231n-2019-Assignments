...
#         {affine - [batch/layer norm] - relu - [dropout]} x (L - 1) - affine - softmax
        ...
        dout , grads['W%d' % (i+1)], grads['b%d' % (i+1)] = affine_backward(dout,caches[p])
        ...
        while p!=-1:
            if self.use_dropout:
                dout = dropout_backward(dout, caches[p])
                p -= 1
                
                
            if self.normalization=='batchnorm':
                dout, grads['W%d' % (i + 1)], grads['b%d' % (i+1)], grads['gamma%d' % (i+1)], grads['beta%d' % (i + 1)] = affine_bn_relu_backward(dout, caches[p])
                
            elif self.normalization=='layernorm':
                dout, grads['W%d' % (i + 1)], grads['b%d' % (i + 1)], grads['gamma%d' % (i + 1)], grads['beta%d' % (i + 1)] = affine_ln_relu_backward(dout, caches[p])
                
            else:   
                dout, grads['W%d' % (i + 1)], grads['b%d' % (i + 1)] = affine_relu_backward(dout, caches[p])
...
def affine_bn_relu_backward( dout , cache ):
    ...
    dout3 = relu_backward(dout , cache3)
    dout2 , dgamma, dbeta = batchnorm_backward( dout3 , cache2)
    dout1 , dw, db = affine_backward(dout2, cache1)
    return dout1, dw, db, dgamma, dbeta

def affine_ln_relu_forward( x,w,b,gamma,beta,bn_param ):
    
    out1 , cache1 = affine_forward( x,w,b )
    out2 , cache2 = layernorm_forward( out1 , gamma ,beta ,bn_param)
    out3 , cache3 = relu_forward(out2 )
    cache = (cache1 , cache2 , cache3)
    return out3, cache
...
def affine_ln_relu_backward( dout , cache ):
    ...
    cache1, cache2,cache3 = cache
    dout3 = relu_backward(dout , cache3)
    dout2 , dgamma, dbeta = layernorm_backward( dout3 , cache2)
    dout1 , dw, db = affine_backward(dout2, cache1)
    return dout1, dw, db, dgamma, dbeta
