...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        score -= score.max()
        score = np.exp(score)
        score_sums = np.sum(score, axis=1)
        cors = score[range(n), y]
        loss = cors / score_sums
        ...
        dz2 = np.divide(score, score_sums.reshape(n, 1))
        dz2[range(n), y] = - (score_sums - cors) / score_sums
        
        dW2 = np.dot( a1.T , dz2 )/n + 2*reg*W2
        grads['W2'] = dW2
